"""
Double Pendulum Web Visualization (PyScript)

This script runs within the browser using PyScript. It coordinates the visualization
of the double pendulum, comparing data generated by the C++ WASM solver with a
real-time Python solver (LSODA).

It handles:
1. Interfacing with the DOM to update status and plots.
2. Reading simulation data generated by the C++ WASM module.
3. Solving the double pendulum equations in Python using SciPy.
4. Computing Lyapunov exponents to quantify chaos.
5. Animating the results using Matplotlib.
"""
import js
import numpy as np
import sympy as smp
